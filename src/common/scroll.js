...
const animate = (() => {
     ...
     */
    result.start = function (stepCallback, verifyCallback, completedCallback, duration, easingMethod, root) {
        const start = time();
        let lastFrame = start;
        let percent = 0;
        let dropCounter = 0;
        ...
        if (id % 20 === 0) {
            const newRunning = {};
            for (const usedId in running) { // eslint-disable-line
        ...
        const step = function (virtual) {
            ...
            if ((stepCallback(value, now, render) === false || percent === 1) && render) {
                ...
                lastFrame = now;
                result.requestAnimationFrame(step, root);
            }
        };
...
const Scroll = function (callback, options) {
    ...
    for (const key in options) {// eslint-disable-line
...
Scroll.prototype = {
    ...
    __stepThroughDeceleration(render) {
        ...
        if (this.options.bouncing) {
            ...
            const {penetrationDeceleration, penetrationAcceleration} = this.options;
