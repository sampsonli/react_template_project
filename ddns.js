...
const config = require('./ddns-config');
...
const { getIp4, getIp6 } = (function () {
    ...
    async function getIp(family, ipApi, regexp) {
        const request = await get(ipApi, undefined, { family });
        let data = '';

        try {
            data = JSON.parse(request).ip;
...
const {
 getRecord4, getRecord6, addRecord4, addRecord6, updateRecord4, updateRecord6,
 // eslint-disable-next-line
} = (function () {
    // 将公共参数和签名附加到请求参数
...
...specParam,
    ...
    async function changeRecord(ip, recordId, type) {
        ...
        const request = await get(config.alidnsAPI, attachParam(parame));
        let data = '';
        try {
            data = JSON.parse(request);
        } catch (e) {
